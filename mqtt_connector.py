...
def main():
    ...
    try:
        ...
        return on_connect_fail(mqtt_client, None, {}, mqtt.MQTT_ERR_NO_CONN, None)

    periodically_publish_BME280_data(mqtt_client)
    mqtt_client.loop_forever(timeout=15, retry_first_connection=True)


def on_connect(client: mqtt.Client, userdata: Any, flags: Dict,
               reason_code: mqtt.ReasonCodes, properties: mqtt.Properties):
    if reason_code == 0:
...
def on_connect_fail(client: mqtt.Client, userdata: Any, disconnect_flags: Dict,
                    reason_code: mqtt.ReasonCodes, properties: mqtt.Properties):
...
def on_disconnect(client: mqtt.Client, userdata: Any, flags: Dict,
                  reason_code: mqtt.ReasonCodes, properties: mqtt.Properties):
...
def periodically_publish_BME280_data(client: mqtt.Client):
    while True:
        try:
            ...
            logging.info(f"Retrying in {MQTT_RECONNECT_INTERVAL} seconds...")
            time.sleep(MQTT_RECONNECT_INTERVAL)
            continue
